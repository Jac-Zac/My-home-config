...
update() {
	# Bar
	url=$(jq -r '.weathergov | "\(.url)\(.location)/\(.format)"' ~/.config/sketchybar/items/weather_config.json)
	weather=$(curl -s "$url")
	...
	# popup
	url=$(jq -r '.wttr | "\(.url)\(.location)?\(.format)"' ~/.config/sketchybar/items/weather_config.json)
